...
namespace mystl
{
	...
	public:
		...
		static void* reallocate(void* p, size_t new_size) {
			...
			if (result == NULL)result = oom_realloc(p, new_size);
			return result;
		}
	...
	template<int inst>
	typename __malloc_alloc_template<inst>::handler __malloc_alloc_template<inst>::__malloc_alloc_oom_handler = 0;
	...
	typedef __malloc_alloc_template<0> _malloc_alloc;
}
